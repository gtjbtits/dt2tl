...
import com.jbtits.github2telegram.domain.event.tlgrm.cbq.ConfigTeamRemoveCallbackEvent;
...
@Slf4j
@Service
public class ConfigTeamRemoveCallbackListener
    extends AbstractTlgrmCallbackQueryListener<TlgrmCallbackContext, ConfigTeamRemoveCallbackEvent> {
  ...
  protected ConfigTeamRemoveCallbackListener(final TlgrmMetaHelper tlgrmMetaHelper,
  ...
  protected TlgrmCallbackQueryAction getAction() {
    return TlgrmCallbackQueryAction.CONFIG_TEAM_REMOVE;
  }

  @Override
  protected @NonNull ConfigTeamRemoveCallbackEvent convertToEvent(final @NonNull CallbackQuery src)
      throws TlgrmListenerException {
    final String data = src.getData();
    final var callbackQueryData = TlgrmCallbackQueryData.decode(data);
    return new ConfigTeamRemoveCallbackEvent(tlgrmMetaHelper.extractCallbackContext(src), callbackQueryData.getArgs());
  }

  @Override
  protected void on(final @NonNull ConfigTeamRemoveCallbackEvent event) {
    ...
    this.configurationKeyValueService.get(context.getChatId()).ifPresent(tribeConfiguration -> {
      this.configurationWizardService.removeTeam(teamName, tribeConfiguration);
